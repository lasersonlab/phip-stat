...
if __name__ == '__main__': mpl.use('agg')   # if running as script, disable any windowing
...
class ParetoFitnessNetwork(FitnessNetwork):
    ...
    def generate_truth(self):
        w_truth = np.random.lognormal(self.mu,self.sigma,self.N)
        theta_truth = np.random.dirichlet(self.alpha*self.Z*w_truth)
        X_truth = np.random.multinomial(self.n,theta_truth)
        return(w_truth,theta_truth,X_truth)
...
class GibbsSamplingAnalysis(object):
    ...
    def __init__(self, Z, X, alpha, iterations, ws, thetas, llws, llths, llXs, lls, frac_accepted):
        ...
        self.extreme_diff = max(np.abs(np.min(diffs)),np.abs(np.max(diffs)))
        self.updates = np.sum(diffs != 0,axis=1)
        self.dirichlet_weights = np.sum(ws*Z*alpha,axis=1)
        self.percentiles_at_1 = [sp.stats.percentileofscore(ws[-500:,i],1) for i in range(N)]
    ...
    def ranked_ws(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'ranked_ws.png'))
    
    def w_distributions_ordered_by_medians(self,output_dir=None):
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.scatter(range(self.N),p5[self.order_by_median_ws], s=5,c='k',lw=0,zorder=1)
        ax.scatter(range(self.N),p95[self.order_by_median_ws],s=5,c='k',lw=0,zorder=1)
        for (pos,low,high) in zip(range(N),p25[self.order_by_median_ws],p75[self.order_by_median_ws]):
            ax.plot([pos,pos],[low,high],color='#bdbdbd',lw=2,zorder=2)
        ax.scatter(range(self.N),p50[self.order_by_median_ws],s=10,c='r',linewidths=0,zorder=3)
        ax.axhline(0,zorder=0)
        ax.set_xlabel('w component (big to small)')
        ax.set_ylabel('w value')
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'w_distributions_ordered_by_medians.png'))
    ...
    def w_distributions_ordered_by_input(self,output_dir=None):
        ...
        ax.set_ylabel('w value')
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'w_distributions_ordered_by_input.png'))
    ...
    def dirichlet_weights(self,output_dir=None):
        ...
        ax.set_ylabel('Z * w')
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'dirichlet_weights.png'))
    ...
    def evolution_w_hist(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'evolution_w_hist.png'))
    ...
    def total_w_weight(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'total_w_weight.png'))
    ...
    def raw_data_by_w(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'raw_data_by_w.png'))
    ...
    def raw_data_by_std(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'raw_data_std.png'))
    ...
    def trajectories_heatmap(self,output_dir=None):
        ...
        ax.set_ylabel('w component')
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'trajectories_heatmap.png'))
    
    def trajectory_derivatives_spy(self,output_dir=None):
        fig = plt.figure(figsize=(self.iterations/250.,self.N/250.))
        ax = fig.add_axes([0.1,0.1,0.87,0.87])
        ax.spy(self.diffs[self.order_by_ws_last,:],aspect='auto')
        ax.set_xlabel('iteration')
        ax.set_ylabel('w component')
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'trajectory_derivatives_spy.png'))
    ...
    def num_updates_vs_median_w(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'num_updates_vs_median_w.png'))
    ...
    def num_updates_vs_std_w(self,output_dir=None):
        ...
        ax.set_ylabel('num updates for that values in %i iterations' % self.iterations)
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'num_updates_vs_std_w.png'))
    ...
    def simulated_data(self,output_dir=None):
        ...
        X_sim = np.random.multinomial(self.n,self.theta)
        fig = plt.figure()
...
class GibbsSamplingAnalysis_with_truth(GibbsSamplingAnalysis):
    ...
    def w_truth_vs_median_w(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'w_truth_vs_median_w.png'))
    ...
    def w_truth_vs_median_w_ranks(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'w_truth_vs_median_w_ranks.png'))
    ...
    def raw_data_by_true_w(self,output_dir=None):
        ...
        if output_dir == None: fig.show()
        else: fig.savefig(os.path.join(output_dir,'raw_data_by_true_w.png'))
